...
use webview2_com::Microsoft::Web::WebView2::Win32::{
    ...
    ICoreWebView2ExecuteScriptCompletedHandler, ICoreWebView2Frame,
    ICoreWebView2FrameCreatedEventHandler, ICoreWebView2Frame2,
    ICoreWebView2PrintToPdfCompletedHandler, ICoreWebView2ScriptDialogOpeningEventHandler,
    ICoreWebView2WebMessageReceivedEventHandler, ICoreWebView2_4, ICoreWebView2_7,
...
unsafe impl Sync for SendableComPtr {}
...
// =============================================================================
// Native frame registry
// =============================================================================

/// Top-level `WebView2` frames captured from `FrameCreated`.
///
/// Upstream drives "switch to frame" by evaluating the script inside
/// `frame.contentWindow`. That only works same-origin: the shell lives on
/// `tauri://localhost` while the embedded harness is served from
/// `http://127.0.0.1:<port>`, so `contentDocument` is `null` and every
/// frame-context script times out (WebDriver `execute/sync` script timeout).
///
/// Holding the native `ICoreWebView2Frame` lets the executor call
/// `ICoreWebView2Frame2::ExecuteScript` instead, which the engine routes to the
/// frame regardless of the origin boundary. Only depth-1 frames are captured;
/// deeper nesting keeps the upstream JS path.
#[derive(Default, Clone)]
pub struct FrameRegistry {
    frames: Arc<Mutex<HashMap<String, Vec<FrameHandle>>>>,
}

/// A frame plus the single COM reference the registry keeps alive.
struct FrameHandle {
    ptr: SendableComPtr,
    name: String,
}
...
impl FrameHandle {
    /// Hand out the frame as a `Send`-safe pointer holding its **own** COM reference.
    ///
    /// The registry must keep its reference: the caller reconstructs the interface inside
    /// `with_webview` and drops it when the closure ends, which Releases it. `clone()`
    /// AddRefs for that future Release, and `into_raw()` returns the registry's own
    /// reference untouched — without this the registry would be left holding a freed
    /// pointer and the next script would crash the process.
    fn acquire(&self) -> SendableComPtr {
        let borrowed = unsafe { ICoreWebView2Frame::from_raw(self.ptr.as_ptr()) };
        let owned = borrowed.clone();
        let _ = borrowed.into_raw();
        SendableComPtr(owned.into_raw())
    }
}

impl FrameRegistry {
    fn register(&self, label: &str, frame: &ICoreWebView2Frame, name: String) {
        let ptr = SendableComPtr(frame.clone().into_raw());
        if let Ok(mut frames) = self.frames.lock() {
            frames
                .entry(label.to_string())
                .or_default()
                .push(FrameHandle { ptr, name });
        }
    }

    fn get(&self, label: &str, index: usize) -> Option<(SendableComPtr, String)> {
        let frames = self.frames.lock().ok()?;
        let handle = frames.get(label)?.get(index)?;
        Some((handle.acquire(), handle.name.clone()))
    }
}

mod frames {
    use webview2_com::Microsoft::Web::WebView2::Win32::{
        ICoreWebView2, ICoreWebView2FrameCreatedEventArgs, ICoreWebView2FrameCreatedEventHandler,
    ...
    #[implement(ICoreWebView2FrameCreatedEventHandler)]
    pub struct FrameCreatedHandler {
        registry: FrameRegistry,
        label: String,
    }

    unsafe impl Send for FrameCreatedHandler {}
    unsafe impl Sync for FrameCreatedHandler {}

    impl FrameCreatedHandler {
        pub fn new(registry: FrameRegistry, label: String) -> Self {
            Self { registry, label }
        }
    }

    impl ICoreWebView2FrameCreatedEventHandler_Impl for FrameCreatedHandler_Impl {
        fn Invoke(
            &self,
            _sender: windows::core::Ref<'_, ICoreWebView2>,
            args: windows::core::Ref<'_, ICoreWebView2FrameCreatedEventArgs>,
        ) -> windows::core::Result<()> {
            ...
            unsafe {
                let Ok(frame) = args.Frame() else {
                    tracing::error!("FrameCreated: args.Frame() failed");
                    return Ok(());
                };

                let name = {
                    let mut ptr = windows::core::PWSTR::null();
                    if frame.Name(&raw mut ptr).is_ok() {
                ...
                tracing::debug!("FrameCreated: name='{name}' registered for '{}'", self.label);
                self.registry.register(&self.label, &frame, name);
            }
...
use frames::FrameCreatedHandler;

/// Register the `FrameCreated` handler so frame handles are captured as they appear.
///
/// # Safety
/// Must be called from a COM-initialized thread with a valid webview.
unsafe fn register_frame_handler(
    webview: &ICoreWebView2,
    registry: FrameRegistry,
    label: &str,
) {
    let Ok(webview4) = webview.cast::<ICoreWebView2_4>() else {
        tracing::error!("CoreWebView2 does not expose ICoreWebView2_4; frames unavailable");
        return;
    };

    let handler: ICoreWebView2FrameCreatedEventHandler =
        FrameCreatedHandler::new(registry, label.to_string()).into();

    let mut token = std::mem::zeroed();
    if let Err(e) = webview4.add_FrameCreated(&handler, &raw mut token) {
        tracing::error!("Failed to register FrameCreated handler: {e:?}");
    } else {
        tracing::debug!("Registered native frame handler for '{label}'");
    }
...
impl<R: Runtime + 'static> WindowsExecutor<R> {
    ...
    ///
    /// Runs `script` in the top-level context, unwrapped.
    async fn evaluate_js_top_level(
        &self,
        script: &str,
        ...
        self.collect_script_result(rx).await
    }

    /// Run `script` inside a native frame via `ICoreWebView2Frame2::ExecuteScript`.
    ///
    /// Unlike the upstream `contentWindow.eval` wrapper this works across origins,
    /// because the engine routes the call to the frame instead of going through the
    /// renderer's same-origin gate.
    async fn evaluate_js_in_frame(
        &self,
        frame_ptr: SendableComPtr,
        script: &str,
    ) -> Result<Value, WebDriverErrorResponse> {
        let (tx, rx) = oneshot::channel();
        let script_owned = script.to_string();
        ...
        let result = self.window.with_webview({
            let tx = tx.clone();
            move |_webview| unsafe {
                let _ = CoInitializeEx(None, COINIT_APARTMENTTHREADED);

                let frame = ICoreWebView2Frame::from_raw(frame_ptr.as_ptr());
                match frame.cast::<ICoreWebView2Frame2>() {
                    Ok(frame2) => {
                        let script_hstring = HSTRING::from(&script_owned);
                        let handler: ICoreWebView2ExecuteScriptCompletedHandler =
                            ExecuteScriptHandler::new(tx.clone()).into();
                        ...
                        if let Err(e) =
                            frame2.ExecuteScript(PCWSTR(script_hstring.as_ptr()), &handler)
                        {
                            tracing::error!("Frame ExecuteScript failed for '{script_preview}...': {e:?}");
                            if let Ok(mut guard) = tx.lock() {
                                if let Some(tx) = guard.take() {
                                    let _ = tx.send(Err(format!("Frame ExecuteScript failed: {e:?}")));
                                }
                            }
                        }
                    }
                    Err(e) => {
                        tracing::error!("Frame does not expose ICoreWebView2Frame2: {e:?}");
                        if let Ok(mut guard) = tx.lock() {
                            if let Some(tx) = guard.take() {
                                let _ = tx.send(Err("Frame does not expose ICoreWebView2Frame2".to_string()));
                            }
                        }
                    }
                }
    ...
    /// Await a script completion channel and shape it like `evaluate_js` expects.
    async fn collect_script_result(
        &self,
    ...
    /// Map a `FrameId` to a slot in the native frame registry.
    ///
    /// The order matches what upstream's JS wrapper walks
    /// (`document.querySelectorAll('iframe, frame')`), so an element reference is
    /// resolved to its DOM position in the top-level document.
    async fn resolve_frame_index(&self, id: &FrameId) -> Result<usize, WebDriverErrorResponse> {
        match id {
            FrameId::Index(index) => Ok(usize::try_from(*index).unwrap_or(usize::MAX)),
                ...
                let script = format!(
                    r"(function() {{
                        ...
                        for (var i = 0; i < frames.length; i++) {{
                            if (frames[i] === el) return i;
                        }}
                        return -1;
                    }})()"
                );
                let result = self.evaluate_js_top_level(&script).await?;
                let index = result.get("value").and_then(Value::as_i64).unwrap_or(-1);

                usize::try_from(index).map_err(|_| WebDriverErrorResponse::no_such_frame())
            }
        }
    }
}
...
pub fn register_webview_handlers<R: Runtime>(webview: &tauri::Webview<R>) {
    ...
    // Native frame handles are captured per window, so the executor can reach into
    // cross-origin frames without the JS `contentWindow.eval` simulation.
    let registry = webview.app_handle().state::<FrameRegistry>().inner().clone();
    ...
    let _ = webview.with_webview(move |webview| unsafe {
        ...
        if let Ok(webview2) = webview.controller().CoreWebView2() {
            ...
            register_frame_handler(&webview2, registry, &label);
        }
    });
}
...
impl<R: Runtime + 'static> PlatformExecutor<R> for WindowsExecutor<R> {
    ...
    async fn evaluate_js(&self, script: &str) -> Result<Value, WebDriverErrorResponse> {
        ...
        // No frame context selected: top-level, unchanged.
        if self.frame_context.is_empty() {
            return self.evaluate_js_top_level(script).await;
        }

        // Only depth-1 frames have a native handle; deeper contexts fall back to the
        // upstream JS wrapper, which is correct (if wasteful) within one origin.
        if self.frame_context.len() == 1 {
            let index = self.resolve_frame_index(&self.frame_context[0]).await?;
            let registry = self.window.state::<FrameRegistry>();
            if let Some((frame_ptr, name)) = registry.get(self.window.label(), index) {
                tracing::debug!("Running script in native frame #{index} (name='{name}')");
                return self.evaluate_js_in_frame(frame_ptr, script).await;
            }
        }

        // Fall back to the upstream JS simulation: either the frame is nested, or the
        // registry has not seen it (non-Windows-origin shell, late frame creation).
        let wrapped = wrap_script_for_frame_context(script, &self.frame_context);
        self.evaluate_js_top_level(&wrapped).await
    }
    ...
    async fn execute_async_script(
        ...
        self.evaluate_js_top_level(&wrapper).await?;
