...
> **This copy is vendored and patched.** See [`PATCH.md`](./PATCH.md). It is
> `tauri-plugin-wdio-webdriver` 1.4.0 from crates.io with one E2E-only change to the
> Windows frame-execution path; everything else is upstream and must stay that way so the
> patch is cheap to drop when upstream supports cross-origin frames.
