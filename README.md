...
**倒角必须远小于它所连接的体量的突出量。** 乳房根部原先用 0.075H，有效半径约 26 mm，
而乳房只前突 43 mm——平滑并集把整个乳房滚回胸廓，剩下一个平胸加乳头，乳下褶还孤零零
留在外面。乳下褶本身也是同一类错误：一根 80 mm 长的胶囊横铺在肋骨上，胸壁沿它的长度
不断退开，而它又完全不跟着乳房走，渲出来就是一道刀口。现在它是一串沿乳房下缘走的浅切球。
...
耳廓要**跨着**颅壁：根埋进去约一厘米，边缘立出来约一厘米——真的耳轮就是这么突出的。
两端都得按「这里实际存在的那个面」来量，用颅骨放样的半宽会差 4 mm（颞部与颊骨把皮肤
顶到了放样外面），耳朵的突出量就小于它自己的并集倒角，平滑并集直接把它吞掉，
光头预设渲出来是个没有耳朵的蛋。现在这个面是沿 +x 步进求出来的。

鼻子的关键数字是**鼻尖前表面**的前突量，不是造鼻尖那颗球的球心偏移：成年人约 21 mm。
把球的半径又加在一个本来就是按表面选的偏移上，鼻尖就跑到 32 mm，侧面成了鹰钩。
诊断靠量 Ricketts E 线（鼻尖—颏前点连线）：当时上唇落在线后 14 mm，正常是 4 mm，
而颏前点相对眉间本来就在正常范围里——所以错的是鼻子。改完是上唇 −6.8 mm、
下唇 −0.5 mm。鼻翼还必须比鼻尖退后将近一厘米；跟鼻尖齐平的话，整个鼻子是块前伸的楔子。
...
- 姿态是固定的解剖学立正位，没有骨骼绑定。
- 上臂内部会各留一个约 8000 顶点的封闭空腔（三角肌与胸大肌之间那道切槽掐出来的），
  由 `bl.keep_largest_part` 删掉。不影响外形。
